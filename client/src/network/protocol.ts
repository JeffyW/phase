 ...
 *
 * Bumps to date:
 *  54 — game_setup and state_update carry GameState, whose WaitingFor can now
 *       be OrderCostReductions (CR 601.2f caster-elected cost-reduction
 *       ordering) and whose actions can be OrderCostReductions. Neither enum
 *       has a serde fallback, so first contact rejects a v53 peer before state
 *       delivery. Bumped in lockstep with full-game protocol 72.
