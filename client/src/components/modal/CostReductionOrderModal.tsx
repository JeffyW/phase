 ...
 * any order." Surfaced when the local player is casting a spell whose reduction
 * order is *observable* — when two legal orders lock in genuinely different
 * total costs.
 *
 * The engine owns ALL of the arithmetic. This component permutes an index array
 * (the `reductions` payload the engine provided) and dispatches the chosen
 * permutation via `GameAction::OrderCostReductions`. It never computes a cost:
 * the total shown for the current arrangement is looked up in the engine's
 * `outcomes` list, and when the arrangement is not itself one of the engine's
 * representatives the total is simply not claimed. No re-derivation from
 * `state.objects`, no mana arithmetic.
 */
export function CostReductionOrderModal() {
  ...
  // Local UI state: the chosen permutation (indices into `reductions`).
  // Seeded from the engine's caster-optimal representative (`outcomes` is
  // sorted cheapest-first) so the default arrangement is the one the engine
  // used to pick silently, and re-seeded whenever a new prompt arrives.
  ...
  const handleConfirm = useCallback(() => {
    dispatch({ type: "OrderCostReductions", data: { order } });
  }, [dispatch, order]);
  ...
  const lockedCost = lockedCostForOrder(order, outcomes);
...
/**
 * The total cost the engine attributed to this exact arrangement, or `null`
 * when the arrangement is not one of the engine's representatives. Returning
 * `null` rather than guessing is the point: the frontend must never assert a
 * cost the engine did not author.
 */
function lockedCostForOrder(
  order: number[],
  outcomes: CostReductionOutcome[],
): ManaCost | null {
